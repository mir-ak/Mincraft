...
const main = () => {
    ...
    let mesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(4000, 4000), groundMaterial);
    ...
    function Selcted(cur) {
        document.getElementById("menu").children[select].classList.remove("Selected");
        if (select + cur > 1) {
            select = 0;
    ...
    function createCube(x, y, z) {
        ...
        let boxMaterial = new THREE.MeshBasicMaterial({color: CubeColor});
    ...
    function mouseClick() {
        ...
        if (intersec.length > 0 && intersec[0].object.typeObject == "block") {
            let elm = intersec[0];
            if (type === "Cube") {
                createCube(elm.object.coord.x + elm.face.normal.x, elm.object.coord.y + elm.face.normal.y, elm.object.coord.z + elm.face.normal.z);
            } else {
                deleteCube(elm);
            }
        } else if (intersec.length > 0 && intersec[0].object.typeObject == "floor" && type != "Hache") {
            let posX = Math.round(intersec[0].point.x / 5);
            let posz = Math.round(intersec[0].point.z / 5);
            createCube(posX, 0, posz, true);
        }
    ...
    function animate() {
        ...
        if (controls.isLocked === true) {
            ...
            let lastPos = new THREE.Vector3(controls.getObject().position.x, controls.getObject().position.y, controls.getObject().position.z);
            let newPos = new THREE.Vector3(object.position.x, controls.getObject().position.y, object.position.z);
            let tmp;
            if (controls.getObject().position.y < 7) {
                velocity.y = 0;
